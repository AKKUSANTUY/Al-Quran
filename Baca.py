...
while True:
    ...
    menud = """\x1b[1;37m
=========================================================
     |  Saya Senang Anda Ingin Membaca Al-Qur'an  |
=========================================================
...
""".format(ayt,ind,art)
    ...
    if menu.lower() == "ar":
	  ...
	  art = False
       ...
       if raw_input ("\nAnda Ingin Membaca Surah "+su+" (y/n) ? ").lower() == "n":
	  break
       os.system("clear")
       try:
        arb = get_arab(su)
       ...
       for sr in surah:
	    ...
	    dikt = {"arb":"<>".join(arb),"indo":"<>".join(indo),"ar":"<>".join(ar)}
            if off[sr] != "":
	          off[sr] = dikt
	          open("quran.txt","w").write(json.dumps(off))
	    sys.stdout.write("\rSedang Download Surah : "+sr+"            \b")
	    sys.stdout.flush()
	    time.sleep(.1)
       print 
